...
class University(models.Model):
    ...
    class Meta:
        verbose_name_plural = "Universities"  # لتصحيح الاسم في لوحة التحكم
        ordering = [
            "-ranking"
        ]  # لترتيب الجامعات حسب التصنيف تلقائياً (من الأعلى للأقل)
...
class Program(models.Model):
    ...
    degree_level = models.CharField(
        max_length=50, choices=DEGREE_CHOICES, verbose_name="الدرجة العلمية"
    )
...
class Requirement(models.Model):
    ...
    req_type = models.CharField(
        max_length=50, choices=REQ_TYPE_CHOICES, verbose_name="نوع الشرط"
    )
...
class Application(models.Model):
    ...
    custom_deadline = models.DateField(
        null=True, blank=True, verbose_name="تاريخ الاستحقاق الشخصي"
    )
...
class Document(models.Model):
    ...
    doc_type = models.CharField(
        max_length=50, choices=DOC_TYPE_CHOICES, verbose_name="نوع الملف"
    )
    file = models.FileField(
        upload_to="application_docs/%Y/%m/%d/", verbose_name="الملف المرفوع"
    )
